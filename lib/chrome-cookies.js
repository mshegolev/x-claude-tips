...
// Только x.com и её поддомены. `host_key LIKE '%x.com'` матчил ещё
// vertex.com, mx.com, linx.com, а имена auth_token/ct0 достаточно общие,
// чтобы чужой session token был расшифрован, записан в файл кук и отправлен
// браузером на x.com.
export const COOKIE_QUERY =
  "SELECT name||'|'||host_key||'|'||hex(encrypted_value) FROM cookies "
  + "WHERE host_key = 'x.com' OR host_key LIKE '%.x.com'";
...
export function extractXCookies(profileDir = DEFAULT_PROFILE) {
  ...
  return withTempCopy(join(profileDir, 'Cookies'), (db) => {
    const rows = execFileSync('sqlite3', [db, COOKIE_QUERY], { encoding: 'utf8' });
