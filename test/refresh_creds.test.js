...
// node:sqlite is built in only from Node 22.5, and refresh_creds.js is the
// twikit-variant helper that guards for its absence itself. Skip the single
// test that needs a real SQLite file rather than failing the whole file on
// the Node 20 runtime the rest of this skill targets.
const hasSqlite = await import('node:sqlite').then(() => true, () => false);
const needsSqlite = hasSqlite
  ? {}
  : { skip: `node:sqlite unavailable on ${process.version} (needs Node >= 22.5)` };
const { DatabaseSync } = hasSqlite ? await import('node:sqlite') : {};

// refresh_creds.js throws at import time when node:sqlite is missing, so the
// whole module is loaded lazily and every test in this file skips together.
const {
  ensureCredsFile,
  parseBrowserOption,
  ...
  readFirefoxCookies,
  X_CREDS_TEMPLATE,
} = hasSqlite ? await import('../refresh_creds.js') : {};

test('ensureCredsFile creates a chmod 600 .x-creds template when missing', needsSqlite, () => {
...
test('parseBrowserOption defaults to firefox and accepts supported browser choices', needsSqlite, () => {
...
test('parseBrowserOption falls back to X_CLAUDE_TIPS_BROWSER, flag wins over env', needsSqlite, () => {
  const saved = process.env.X_CLAUDE_TIPS_BROWSER;
...
test('readFirefoxCookies reads auth_token and ct0 from a copied cookies.sqlite DB', needsSqlite, () => {
...
test('parseSafariBinaryCookies reads auth_token and ct0 from a binarycookies buffer', needsSqlite, () => {
